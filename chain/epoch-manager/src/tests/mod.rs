...
fn test_protocol_version_switch_with_shard_layout_change() {
    ...
    let protocol_version = epoch_manager.get_epoch_info(&epochs[1]).unwrap().protocol_version();
    assert_eq!(epoch_manager.get_shard_layout(protocol_version), ShardLayout::single_shard());
    assert_eq!(
        epoch_manager.get_epoch_info(&epochs[2]).unwrap().protocol_version(),
        new_protocol_version
    );
    let protocol_version = epoch_manager.get_epoch_info(&epochs[2]).unwrap().protocol_version();
    assert_eq!(
        epoch_manager.get_shard_layout(protocol_version),
